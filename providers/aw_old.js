...
var provider = {  
   ...
   getSRSS: function(callback) {
    ...
    parseResponse: function(response) {
        ...
        var current = {current:{
        	weather: result[0].DailyForecasts.Day.IconPhrase,
            temp_c: Math.round(result[0].DailyForecasts.Temperature.Maximum),
            temp_c_min: Math.round(result[0].DailyForecasts.Temperature.Minimum),
            temp_c_feels: Math.round(result[0].DailyForecasts.RealFeelTemperature.Maximum),
            temp_f: Math.round(result[0].Temperature.Imperial.Value),
            icon: this.imageArray[result[0].DailyForecasts.Day.Icon],
            relative_humidity: result[0].RelativeHumidity,               //Get from current
            pressure_in: Math.round(result[0].Pressure.Imperial.Value),  //na
            pressure_mb: result[0].value,                                //na
            UV: "123",
            wind_mph: result[0].Wind.Speed.Imperial.Value,
            wind_kph: result[0].DailyForecasts.Day.Wind.Speed.Value,
            visibility_mi: "n/a",                                        //na
            visibility_km: "n/a",                                        //na
            cloudCover: result[0].DailyForecasts.Day.CloudCover,
            rainProb: result[0].DailyForecasts.Day.PrecipitationProbability,
            rainMM: result[0].DailyForecasts.Day.Rain.Value,
            rainHoursd: result[0].DailyForecasts.Day.HoursOfRain,
		   forecast:{
